...
class StrategyEngine:
    ...
    def analyze_ema_crossover(
        ...
        return {
            ...
            "ema50": ema50,
            "reason": (
                f"EMA20({'>' if ema20 > ema50 else '<'})EMA50 - "
                f"{abs(ema20 - ema50):.2f} spread"
            ),
        }
    ...
    def analyze_atr_volatility(
        ...
        # Always initialize reason so normal volatility cannot crash
        # strategy evaluation.
        reason = (
            f"Normal Volatility ({atr_percent:.2f}%) - "
            "No volatility expansion"
        )

        # High volatility = possible breakout environment.
        ...
        # Low volatility = consolidation.
            ...
            signal_strength = 0.2
            reason = (
                f"Low Volatility ({atr_percent:.2f}%) - "
                "Consolidation"
            )
...
class AIDecisionLayer:
    ...
    def make_decision(self, market_data: Dict) -> Dict:
        ...
        rsi_analysis = self.strategy.analyze_rsi(
            indicators.get("rsi", 50)
        )

        atr_analysis = self.strategy.analyze_atr_volatility(
            indicators.get("atr", 5),
            price,
        )

        momentum_analysis = self.strategy.analyze_momentum(
            price_history
        )
        ...
        signals = [
            (
                ...
                self.strategy.weights["ema_crossover"],
            ),
            (
                rsi_analysis["signal"],
                rsi_analysis["strength"],
                self.strategy.weights["rsi"],
            ),
            (
                atr_analysis["signal"],
                atr_analysis["strength"],
                self.strategy.weights["atr"],
            ),
            (
                momentum_analysis["signal"],
                momentum_analysis["strength"],
                self.strategy.weights["momentum"],
            ),
        ]
        ...
        final_decision = self._score_to_decision(
            buy_score,
            sell_score,
        )
        ...
        logger.info(
            f"Decision made: {final_decision} "
    ...
    def _get_recommendation(
        ...
        if decision in [
            ...
            return {
                "action": "HOLD/WAIT",
                "reason": (
                    "No clear signal - awaiting confirmation"
                ),
