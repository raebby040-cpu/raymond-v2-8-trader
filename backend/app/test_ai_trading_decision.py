...
def test_bullish_context_produces_buy_proposal():
    ...
    decision = engine.evaluate(_bullish_context())
...
def test_bearish_context_produces_sell_proposal():
    ...
    decision = engine.evaluate(_bearish_context())
...
def test_neutral_context_produces_wait():
    ...
    decision = engine.evaluate(_neutral_context())
...
def test_risk_reward_meets_minimum():
    ...
    assert decision.proposal.risk_reward >= 1.5

def test_adaptive_risk_reward_respects_configured_maximum():
    ...
    assert decision.proposal.risk_reward <= 2.0

def test_adaptive_risk_reward_respects_configured_minimum():
    ...
    assert decision.proposal.risk_reward >= 1.8
...
def test_serialization_contains_safety_flags():
    ...
    data = ai_decision_to_dict(decision)
    ...
    assert data["proposal"]["direction"] == "buy"

    assert data["proposal"]["execution_type"] == "paper"

    assert data["proposal"]["broker_order_required"] is False
...
def test_wait_decision_has_no_trade_proposal():
