...
class TestStrategyEngine:
    ...
    def test_atr_normal_volatility(self, strategy_engine):
        """Test ATR normal volatility does not fail."""
        result = strategy_engine.analyze_atr_volatility(
            atr=10.0,
            price=2050.0,
            historical_atr=10.0,
        ...
        assert "Normal Volatility" in result["reason"]
    ...
    def test_momentum_insufficient_data(self, strategy_engine):
        ...
        result = strategy_engine.analyze_momentum(
            [2050.0, 2051.0]
        )
        assert result["signal"] == SignalType.HOLD
...
class TestAIDecisionLayer:
    ...
    def test_make_decision_strong_buy(self, ai_decision_layer):
        ...
        decision = ai_decision_layer.make_decision(
            market_data
        )
    ...
    def test_recommendation_includes_levels(
        self,
        ai_decision_layer,
    ):
    ...
    def test_decision_history_tracking(
        ...
        history = ai_decision_layer.get_decision_history(
            limit=10
        )
